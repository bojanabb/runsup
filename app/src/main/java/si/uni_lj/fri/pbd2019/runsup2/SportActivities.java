...
final class SportActivities {
    ...
    public static double countCalories(int sportActivity, final float weight, List<Float> SpeedList, double timeFillingSpeedListInHours) {
        float avg=avgList(SpeedList);
        double met = getMET(sportActivity, avg);
        float defweight=60;
        double cal = met*defweight*timeFillingSpeedListInHours;
        return cal;
    }
}
